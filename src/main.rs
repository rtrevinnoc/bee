...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    impl Scraper for Explorer {
        ...
        fn scrape(
            ...
            match response.html().select(&self.meta_description_selector).next() {
                ...
                Some(value) => { description = value.value().attr("content").unwrap().trim().to_string(); }
    ...
    while let Some(output) = collector.next().await {
        if let Ok((url, title, header, description, depth)) = output {
            println!("Visited {}[{}][{}]{{{}}} at depth: {}", url, title, header, description, depth);
        }
    }
